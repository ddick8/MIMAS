...
if(file.exists(paste("data/species_maps/breeding/",gsub(" ", "_", spec_to_download),"_breeding_all",sep="",".csv"))){
  ...
  get_spec_maps_new(spec_to_download,with_error_maps = FALSE)
}
```
...
start_date <- (ebirdst_runs[which(ebirdst_runs$common_name==species_target),6:7])
...
library(dplyr)
library(ggplot2)
single_bird <- locs_rec %>% filter(ID == 2)
...
```
```{r}
# Other visualizations for understanding the model
library(dplyr)
library(ggplot2)
library(sf)
...
glimpse(simulationData)

simulationData <- locs_rec %>% filter(Season == 3)

p <- ggplot() + coord_map("mollweide",xlim=c(-130,-80),ylim=c(15,70)) +
  geom_polygon(data=countries, aes(x=long, y=lat, group=group), fill="grey90", color="darkgrey") +
  geom_polygon(data=states, aes(x=long, y=lat, group=group), fill="grey90", color="darkgrey") +
  geom_point(data=simulationData,aes(x=Lon,y=Lat,col=as.factor(Season))) +
  scale_colour_manual(values=c("firebrick3","goldenrod2","skyblue3","seagreen"),
                      labels=c("Breeding","Post-breeding migration","Wintering","Pre-breeding migration")) +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) +
  theme(axis.ticks.x = element_blank(),axis.text.x = element_blank(),
...
ggplot(simulationData, aes(x = Day)) +
...
ggplot(simulationData, aes(x = Lat)) +
  geom_histogram(binwidth = 0.5, fill = "blue", color = "black") +
  labs(title = "Histogram of Values",
       x = "Values",
       y = "Frequency") +
...
#%>% filter(Season == 2 | Season == 4) %>%
data <- locs_rec %>% mutate(SeasonFactor = recode_factor(Season,
                                      `1` = "Breeding",
                                      `2` = "Post-breeding migration",
                                      `3` = "Wintering",
                                      `4` = "Pre-breeding migration"))

ggplot(data, aes(x = Lon, fill = SeasonFactor)) +
  geom_histogram(binwidth = 0.5) +
  labs(title = "Histogram of longitude Colored by Season",
       x = "Lon",
       y = "Frequency") +
   scale_fill_manual(values=c("firebrick3","goldenrod2","skyblue3","seagreen"),
                     labels=c("Breeding","Post-breeding migration","Wintering","Pre-breeding migration")) +
  theme_minimal()

ggplot(data, aes(x = Lat, fill = SeasonFactor)) +
  geom_histogram(binwidth = 0.5) +
  labs(title = "Histogram of latitude Colored by Season",
       x = "Lat",
       y = "Frequency") +
   scale_fill_manual(values=c("firebrick3","goldenrod2","skyblue3","seagreen"),
                     labels=c("Breeding","Post-breeding migration","Wintering","Pre-breeding migration")) +
